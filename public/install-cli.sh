...
NODE_VERSION="${NAGABOT_NODE_VERSION:-22.22.0}"
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${NAGABOT_NPM_LOGLEVEL:-error}"
JSON=0
RUN_ONBOARD=0
SET_NPM_PREFIX=0
...
Usage: install-cli.sh [options]
  ...
  --prefix <path>        Install prefix (default: ~/.nagabot)
  --version <ver>        Nagabot version (default: latest)
  --node-version <ver>   Node version (default: 22.22.0)
  --onboard              Run "nagabot onboard" after install
  --no-onboard           Skip onboarding (default)
  --set-npm-prefix       Force npm prefix to ~/.npm-global if current prefix is not writable (Linux)

Environment variables:
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)
  NAGABOT_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
EOF
}
