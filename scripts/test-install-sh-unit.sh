...
export NAGABOT_INSTALL_SH_NO_RUN=1
...
(
  ...
  make_exe "${bin}/nagabot" 'echo "ok" >/dev/null'
  export PATH="${bin}:/usr/bin:/bin"

  got="$(resolve_nagabot_bin)"
  assert_eq "$got" "${bin}/nagabot" "resolve_nagabot_bin (direct PATH)"
)
...
(
  ...
  make_exe "${prefix}/bin/nagabot" 'echo "ok" >/dev/null'

  export PATH="${tool_bin}:/usr/bin:/bin"

  got="$(resolve_nagabot_bin)"
  assert_eq "$got" "${prefix}/bin/nagabot" "resolve_nagabot_bin (npm prefix -g)"
)
...
set -euo pipefail
if [[ "\${1:-}" == "rehash" ]]; then
  cat >"${shim}/nagabot" <<'SHIM'
#!/usr/bin/env bash
set -euo pipefail
echo ok >/dev/null
SHIM
  chmod +x "${shim}/nagabot"
  exit 0
fi
exit 0
EOF
  ...
  command -v nagabot >/dev/null 2>&1 && fail "precondition: nagabot unexpectedly present"

  got="$(resolve_nagabot_bin)"
  assert_eq "$got" "${shim}/nagabot" "resolve_nagabot_bin (nodenv rehash)"
)

echo "==> case: warn_nagabot_not_found (smoke)"
(
  ...
  out="$(warn_nagabot_not_found 2>&1 || true)"
  assert_nonempty "$out" "warn_nagabot_not_found output"
)
